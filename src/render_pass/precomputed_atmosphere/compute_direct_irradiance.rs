...
pub fn get_framebuffer_data_create_info(project_renderer: &ProjectRenderer) -> FramebufferDataCreateInfo {
    let render_target0 = project_renderer.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_DELTA_IRRADIANCE);
    let render_target1 = project_renderer.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_IRRADIANCE);
...
pub fn get_render_pass_data_create_info(project_renderer: &ProjectRenderer) -> RenderPassDataCreateInfo {
    let render_pass_name = String::from("compute_direct_irradiance");
