...
pub fn get_framebuffer_data_create_info(project_renderer: &ProjectRenderer) -> FramebufferDataCreateInfo {
    let render_target0 = project_renderer.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_DELTA_RAYLEIGH_SCATTERING);
    let render_target1 = project_renderer.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_DELTA_MIE_SCATTERING);
    let render_target2 = project_renderer.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_SCATTERING);
    let render_target3 = project_renderer.get_render_target(RenderTargetType::PRECOMPUTED_ATMOSPHERE_OPTIONAL_SINGLE_MIE_SCATTERING);
...
pub fn get_render_pass_data_create_info(project_renderer: &ProjectRenderer) -> RenderPassDataCreateInfo {
    ...
    let pipeline_data_create_info = PipelineDataCreateInfo {
        ...
        _pipeline_fragment_shader_file: PathBuf::from("precomputed_atmosphere/compute_single_scattering.frag"),
